package TaskManager.scripts.crafting;
...
public class WoolSpinner extends Script {
	...
	private State getState() {
		if (BANK_AREA.contains(Players.getLocal().getTile()) && !Inventory.contains(WOOL)
				&& !Inventory.contains(BALL_OF_WOOL) && Inventory.emptySlotCount() < 28
				|| BANK_AREA.contains(Players.getLocal().getTile()) && !Inventory.contains(WOOL)
						&& Inventory.contains(BALL_OF_WOOL)) {
			return State.BANKDEPOSIT;
		}

		if (BANK_AREA.contains(Players.getLocal()) && Inventory.emptySlotCount() == 28) {
			return State.BANKWITHDRAW;
		}

		if (Inventory.count(WOOL) > 0 && !SPINNER_AREA.contains(Players.getLocal())) {
			return State.WALK_TO_SPINNER;
		}
		if (SPINNER_AREA.contains(Players.getLocal()) && Inventory.contains(WOOL)) {
	...
	public int onLoop() {
		...
		case WALK_TO_SPINNER:
			if (Players.getLocal().isMoving() && Walking.getDestinationDistance() > 2)
				break;
			if (Players.getLocal().getTile().getZ() == 2) {
				if (TOP_STAIRS.contains(Players.getLocal().getTile())) {
			...
			} else if (Players.getLocal().getTile().getZ() == 1) {
				if (!Map.canReach(SPINNER_TILE)) {
					...
					Sleep.sleepUntil(() -> Players.getLocal().isMoving(), Calculations.random(1000, 1500));
					Sleep.sleepWhile(() -> Players.getLocal().isMoving(), Calculations.random(4000, 6000));
					...
					Sleep.sleepUntil(new Condition() {
						...
						public boolean verify() {
							...
							return Players.getLocal().distance(SPINNER_TILE) < 4;
						}
		...
		case SPIN_WOOL:
			...
			if (!isAnimating() && child1 == null && !Players.getLocal().isMoving()) {
				currentStage = "Opening spinning options.";
				GameObject spinner = GameObjects.closest("Spinning wheel");
				...
				sleep(1000, 2000);
				child1 = Widgets.getWidgetChild(270, 14);
				Sleep.sleepWhile(() -> child1 == null || !child1.isVisible(), Calculations.random(1000, 2000));
			}
			...
			if (Players.getLocal().getTile().getZ() == 2) {
				...
				Sleep.sleepUntil(new Condition() {
					...
					public boolean verify() {
						...
						return Players.getLocal().distance(BANK_TILE) < 4;
					}
				}, Calculations.random(3000, 6000));
			} else if (Players.getLocal().getTile().getZ() <= 1) {
				if (!Map.canReach(new Tile(3205, 3209, 1))) {
					...
					if (Players.getLocal().distance(stairs) > 3)
					{
						currentStage = "Walking to stairs.";
	...
	public boolean isAnimating() {
		return (Players.getLocal().isAnimating() || animDelay.elapsed() < 3000);
	}
