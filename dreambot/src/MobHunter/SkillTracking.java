...
public class SkillTracking {
	...
	public SkillTracking(AbstractScript as) {
		this.as = as;
		for (int i = 0; i < skillDelays.length; i++)
			as.getSkillTracker().start(Skill.forId(i));
	}
	
	public void refresh() {
		for (int i = 0; i < skillDelays.length; i++) {
				if (as.getSkillTracker().getGainedExperience(Skill.forId(i)) > 0) {
					as.getSkillTracker().reset(Skill.forId(i));
	...
	public void onPaint(Graphics2D g, Skills sk, Timer totalTime, int xPosition) {
		...
		for (int i = 0; i < skills.length; i++) {
			...
			if (decay <= 6000 && totalTime.elapsed() >= 4000) {
				...
				int level = sk.getRealLevel(skill);
				int xpNeeded = Utilities.getXPForLevel(level + 1) - Utilities.getXPForLevel(level);
				int xp = sk.getExperience(skill) - Utilities.getXPForLevel(level);
				int alpha = (decay > 5000 ? (int) (255 - ((100.0 / 1000.0 * (decay - 5000)) / 100.0 * 255)) : 255);
				drawSkillProgress(g, xPosition - 1, 72 + (needToDisplay * 15) + (needToDisplay * 3), xp, xpNeeded, skill.getId(), level, alpha);
				needToDisplay++;
			}
		}
	}
